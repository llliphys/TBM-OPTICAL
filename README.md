...
$$\sigma_{\alpha\beta}(\omega) = \frac{e^2\hbar}{2\pi^2i}\sum_{m, n}\int d\textbf{k} \frac{f[E_m(\textbf{k})] - f[E_n(\textbf{k})]}{E_m(\textbf{k}) - E_n(\textbf{k})} \frac{<m\textbf{k}|\textbf{v}_{\alpha}|{n\textbf{k}><n\textbf{k}|\textbf{v}_{\beta}|m\textbf{k}>}{E_m(\textbf{k}) - E_n(\textbf{k}) + \hbar\omega + i\eta},$$
